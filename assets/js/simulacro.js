...
function renderRecordExamen() {
    if (!progreso.x.length) {
        $('exam-record').innerHTML = `<div class="desglose__item" style="grid-column:1/-1"><div class="desglose__modulo">${T.exam.sinIntentos}</div></div>`;
        return;
    }
...
function iniciarExamen() {
    ...
    examTimerId = setInterval(() => {
        ...
        $('exam-timer').classList.toggle('cronometro--alerta', examRestante <= 120);
        if (examRestante <= 0) finalizarExamen(true);
    }, 1000);
    $('exam-timer').textContent = formateaTiempo(examRestante);
    $('exam-timer').classList.remove('cronometro--alerta');
...
function renderExamPregunta() {
    ...
    $('exam-panel').innerHTML = `
        <div class="quiz__meta">${MODULOS[p.m]} · ${p.tipo} · ${T.exam.respondidas(respondidas, examPreguntas.length)}</div>
        <h3 style="margin-bottom:12px;">${p.q}</h3>
        ${p.code ? `<pre class="codigo">${marcarHueco(p.code)}</pre>` : ''}
...
function finalizarExamen(porTiempo) {
    ...
    $('exam-results').innerHTML = `
        <div class="panel resumen">
            ...
            <div class="resumen__detalle">${T.ui.correctas(ok, examPreguntas.length)}${sinResponder ? T.ui.sinResponderN(sinResponder) : ''}</div>
            <p>${scoreMessage(pct)}</p>
            <div class="desglose">${renderBreakdown(porModulo)}</div>
            <button class="boton-accion" id="exam-again" style="margin-top:20px;">${T.exam.nuevo}</button>
        </div>
        ${fallos.length ? `
        <div class="panel">
            ...
            ${fallos.map(x => `
                <div class="quiz__pregunta">
                    ...
                    <h3>${x.p.q}</h3>
                    ${x.p.code ? `<pre class="codigo">${marcarHueco(x.p.code)}</pre>` : ''}
                    <div class="quiz__opciones">
                        <div class="quiz__opcion quiz__opcion--correcta"><span>✓ ${x.p.options[x.p.correct]}</span></div>
                        ${x.elegida !== -1 ? `<div class="quiz__opcion quiz__opcion--incorrecta"><span>✗ ${T.exam.tuRespuesta} ${x.p.options[x.elegida]}</span></div>`
                                           : `<div class="quiz__opcion"><span>${T.exam.sinResponder}</span></div>`}
                    </div>
                    <div class="quiz__explicacion quiz__explicacion--visible">${x.p.explanation}</div>
                </div>`).join('')}
        </div>` : `<div class="panel resumen"><p>${T.exam.sinFallos}</p></div>`}`;

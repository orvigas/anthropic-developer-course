...
Developer Certification · Module 3
...
- Permission Modes & Human Gates3 screens
- Teaching
- Watch Out
- Checkpoint

- Durable Project Context3 screens
- Teaching
- Watch Out
- Checkpoint

- Packaging Workflows4 screens
- Teaching
- Checkpoint
- Watch Out
- Checkpoint

- MCP Servers3 screens
- Teaching
- Watch Out
- Checkpoint

- Enterprise Integration3 screens
- Teaching
- Watch Out
- Checkpoint

- Cumulative Integration Task2 screens
- Bug ID
- Assembly
...
- Module Complete1 screen
- Module Complete

21 screens · 8 sections · 142 minutes · 8 checkpoints
...
`[TAG TEACHING]` TeachingPermission Modes & Human Gates·17 min
...
`[TAG FAILURE]` Watch OutPermission Modes & Human Gates·3 min
...
`[TAG CHECKPOINT]` CheckpointPermission Modes & Human Gates·4 min
...
`[TAG FAILURE]` Watch OutDurable Project Context·4 min
...
Session context window loaded:
 CLAUDE.md: 847 lines
 Contents include: framework preferences (lines 1–40), testing conventions (lines 41–90), style guide (lines 91–210), dependency rules (lines 211–320), path restrictions (lines 321–360), historical decisions log (lines 361–700), archived notes (lines 701–847)User prompt:"Refactor the auth module to use the new token service. Do not modify the /legacy/tokens/ directory."Claude Code action:[Reading auth module ...]Claude Code action:[Editing auth/service.ts ...]Claude Code action:[Reading /legacy/tokens/store.ts for reference ...]Claude Code action:[Editing /legacy/tokens/store.ts to update token interface ...]
Path restriction from CLAUDE.md (line 347):"Do not modify files in /legacy/tokens/."User:"You just edited /legacy/tokens/store.ts. I said not to touch that directory."Claude Code:"I apologize. The /legacy/tokens/ restriction is in the project instructions. The edit was made in the course of updating the token interface. I did not flag the conflict before proceeding."
...
`[TAG CHECKPOINT]` CheckpointDurable Project Context·3 min
...
Blank 2: the command
A script that reads the tool call from stdin, checks the file path, and exits with code 2 when the path is .env.production (writing the reason to stderr).A script that logs the tool call to an audit file and exits 0.A script that prints a warning and exits 0 unconditionally.`[TAG TEACHING]` TeachingPackaging Workflows·8 min
...
`[TAG CHECKPOINT]` CheckpointPackaging Workflows·3 min
...
`[TAG CHECKPOINT]` CheckpointPackaging Workflows·4 min
...
---
## Steps
1. Run the validation script:/Users/alexmorgan/projects/deploy-utils/validate.shabsolute path2. If the script exits with a non-zero code, report the error to the developer.
...
AReference the script from the project root using CLAUDE_PROJECT_DIR, so it resolves no matter where the project is cloned.BReplace the path with another absolute path that points to a shared network drive.CReplace the path with a home-directory shortcut: ~/projects/deploy-utils/validate.sh.DRemove step 1 so the skill no longer calls an external script.`[TAG TEACHING]` TeachingMCP Servers·21 min
...
`[TAG FAILURE]` Watch OutMCP Servers·4 min
...
Before (do not use)
{
 ...
 "headers": {
 "Authorization": "Bearersk-abc123...".............inline credential}
}
After (correct)
{
 "type": "http",
...
`[TAG CHECKPOINT]` CheckpointMCP Servers·4 min
...
A local SQLite query tool you use only on your development machine.HTTP + Project (.mcp.json)HTTP + Enterprise (managed settings)stdio + Localstdio or HTTP + LocalA code search service hosted on your company’s infrastructure that the whole engineering team should access.HTTP + Project (.mcp.json)HTTP + Enterprise (managed settings)stdio + Localstdio or HTTP + LocalAn experimental web-scraping server you are testing this week against one specific repository, not ready to share.HTTP + Project (.mcp.json)HTTP + Enterprise (managed settings)stdio + Localstdio or HTTP + LocalA security-scanning server your organization’s IT team needs deployed to every developer’s Claude Code installation.HTTP + Project (.mcp.json)HTTP + Enterprise (managed settings)stdio + Localstdio or HTTP + Local`[TAG TEACHING]` TeachingEnterprise Integration·18 min
...
`[TAG FAILURE]` Watch OutEnterprise Integration·3 min
...
`[TAG CHECKPOINT]` CheckpointEnterprise Integration·3 min
...
[MCP Client] GET /auth/token,401 Unauthorized[MCP Client] Reading credential from: /home/jenkins/.config/mcp-credentials.json
[MCP Client] Credential value: WAREHOUSE_TOKEN= sk-****[redacted]
[MCP Client] Retrying with credential,401 Unauthorized[MCP Client] Connection failed after 3 attemptsAFix A: Rotate the API key and update /home/jenkins/.config/mcp-credentials.json with the new value.BFix B: Rotate the rejected key, then move the credential out of the file and inject it as an environment variable in the CI pipeline runner configuration. Update the MCP configuration to reference the variable.CFix C: Switch from API key authentication to OAuth for this service.`[TAG CUMULATIVE]` CumulativeCumulative Integration Task·6 min
...
---
## Steps
1. Run:/Users/priya/scripts/validate-migration.sh2. Report validation results.
File 3: .mcp.json
{
 "mcpServers": {
 "data-warehouse": {
 ...
 "url": "https://warehouse.internal/mcp",
 "headers": {
 "Authorization": "Bearersk-prod-warehouse-abc123"
 }
 }
 }
}Model answer
...
`[TAG CUMULATIVE]` CumulativeCumulative Integration Task·6 min
...
`[TAG MODULE]` RecapKey Takeaways·6 min
# Seven key takeaways
...
Start Module 4 →Return to course home
## Module 3 complete.
Screen 22 of 22

---

*Fuente: Claude Code, MCP & Integration_files/Developer_M3_vF2.html*
